...
func (c *DeepSeekClient) GenerateFairyTale(theme string, date string, style []string) (*response.Story, error) {
    ...
    1. 故事包含3-4个章节，每个章节字数在200字左右
    2. 为整个故事推荐一个背景音乐风格
    3. 今天是%s，请确保故事内容新颖不重复，但故事中不要出现明确的时间信息
	4. 每个章节需要提供详细的图片描述，用于AI绘画
    ...
    6. 图片描述中，所有图片描述必须使用相同风格，风格根据故事内容从以下风格中选出：%s
	7. 所给图片描述中除第一张外，其他图片添加以下提示词：“参考这个图片的风格，以[人物描述]为主角，讲述[故事内容]的故事，适合2-8岁儿童的绘本故事，绘制如下场景：[图片描述]”
    请以JSON格式返回，包含以下字段：
    - title: 故事标题
    - author: 作者(可以虚构)
    - description: 故事简介
